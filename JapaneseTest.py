...
def curr_particles():
    ...
    input(
    "For example these are the most common particles:\n"
    f"{'\n'.join(f'{particle}: {meaning}' for particle, meaning in particles_ques.items())}"
    "\nPress Enter to continue..."
    )
    ...
    while True:
        ...
        if currparticl_ques.strip().lower() == "yes":
            particles_question
            return
        elif currparticl_ques.strip().lower() == "no":
            return
        else:
            print("Choice needs to be either yes or no.")
            continue
def particles_question():
    pass
def stop_prog():
...
question_choice = {
    ...
    "5": {
        ...
        "options": {
            ...
            "3": {
                "name": "Particles quiz 1",
                "worker": particles_question,
                "log": True
            }
